...
// Day-of reminder — sent once, the morning of a confirmed booking.
export async function notifyBookingReminder(booking) {
  await prisma.notification.create({
    data: {
      ...
      audience: "customer",
      type: "reminder",
      title: "Reminder: Booking Today",
      message: `Just a reminder — your booking today at ${booking.startTime} in ${booking.location.name} is coming up.`,
    },
  });
  await emailCustomerReminder(booking);
}
